...
1. 소비 내역 데이터 처리 </br?
  ...
  - 분류된 소비 데이터는 사용자 메타데이터(사용자명, 계좌번호, 거래 내역 테이블 이름)를 저장하는 user_metadata 테이블과 함께 
    각 사용자별로 동적으로 생성된 테이블에 SQLite 데이터베이스에 저장됩니다.
...
3. 사용자 시나리오 처리
소비 분석과 카드 추천 과정에서 다양한 사용자 시나리오를 처리할 수 있도록 쿼리 분류 시스템을 설계하였습니다.
