...
# 한글 폰트 설정
plt.rc('font', family='MaruBuri-Regular') # For Windows
...
def plot_top_category(df, k):
    ...
    plt.bar(top_categories['mid'], top_categories['출금금액'], color='navy')
    ...
    # 파일로 저장
    plot_path = "top_category_plot.png"
    plt.savefig(plot_path)
    plt.close()  # 메모리 절약을 위해 닫기
    return plot_path  # 파일 경로 반환
...
def plot_monthly_consumption(df, year):
    # 연도별 월별 소비내역 필터링
    monthly_consumption = montly_consumption(df, year)
    # 그래프 생성
    plt.figure(figsize=(10, 6))
    custom_palette = ["navy", "orange"]
    plt.bar(monthly_consumption['월'], monthly_consumption['출금금액'], color=custom_palette[0])

    plt.xlabel('월')
    plt.ylabel('출금금액')
    plt.title(f'월 별 소비내역 ({year}년)')
    plt.xticks(monthly_consumption['월'])
    plt.grid(axis='y', linestyle='--', alpha=0.7)

    # 파일로 저장
    plot_path = "monthly_consumption_plot.png"
    plt.savefig(plot_path)
    plt.close()  # 메모리 절약을 위해 닫기
    return plot_path  # 파일 경로 반환
